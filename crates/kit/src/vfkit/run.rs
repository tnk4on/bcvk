...
pub fn run(opts: VmRunOpts) -> Result<()> {
    ...
    if !Path::new(&ssh_key_path).exists() {
        bail!("SSH key not found: {}. Specify with --ssh-key", ssh_key_path);
    }
    ...
    let mut vfkit_args = vec![
        ...
        "--device".to_string(), format!("virtio-blk,path={}", opts.disk),
        "--device".to_string(),
        format!("virtio-net,unixSocketPath={},mac={}", gvproxy_sock_str, mac_str),
        "--device".to_string(),
...
fn find_ssh_key() -> Result<String> {
    let home = dirs::home_dir().ok_or_else(|| color_eyre::eyre::eyre!("cannot determine home directory"))?;
