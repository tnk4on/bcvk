...
pub fn run(opts: VmStartOpts) -> Result<()> {
    ...
    let mut vfkit_args = vec![
        "--cpus".to_string(), meta.cpus.to_string(),
        "--memory".to_string(), meta.memory.to_string(),
        "--bootloader".to_string(),
        format!("efi,variable-store={},create", meta.efi_store),
        "--device".to_string(), format!("virtio-blk,path={}", meta.disk_image),
        "--device".to_string(),
    ...
    for attempt in 0..15u32 {
        match expose_ssh_port(&services_sock_str, "192.168.127.2", meta.ssh_port) {
            Ok(_) => { info!("SSH port {} forwarded", meta.ssh_port); break; }
            Err(e) if attempt < 14 => {
