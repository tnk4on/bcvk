//! Ephemeral VM launch flow for macOS using libkrun C API + ProxyFs (vsock relay).
//!
//! Boot flow:
//! 1. Extract kernel + initramfs from container image
//! 2. Append bcvk units CPIO to initramfs (/etc overlay + /var tmpfs + SSH)
//! 3. Launch VM via libkrun with VirtioFS (ProxyFs) + virtio-net (gvproxy)
//! 4. Start proxy-fsd in podman-machine to serve container rootfs over vsock
//!
//! No SquashFS creation needed — ProxyFs provides zero-copy access to container rootfs.
//! Requires custom libkrun (ProxyFs support) + custom podman (vsock port 5001).
...
// --- libkrun C API FFI (requires custom libkrun with ProxyFs support) ---
...
extern "C" {
    ...
    fn krun_add_virtiofs_proxy(
        ctx_id: u32, tag: *const std::ffi::c_char, socket_path: *const std::ffi::c_char,
    ) -> i32;
...
pub fn run(opts: RunEphemeralOpts) -> Result<()> {
    ...
    info!(image = %opts.image, "starting ephemeral VM on macOS (libkrun + ProxyFs/vsock)");
    ...
    // Step 1: Extract kernel + initramfs
    ...
    // 4. Build kernel cmdline (VirtioFS root, not SquashFS)
    let mut cmdline_parts: Vec<&str> = vec![
        "rootfstype=virtiofs", "root=rootfs", "rw",
        "console=hvc0", "loglevel=4",
        "rd.driver.pre=virtio_mmio", "rd.driver.pre=virtio_net",
    ...
    // 5. Launch VM via libkrun with ProxyFs
    ...
    let relay_sock = find_bcvk_fsd_socket(&machine)?;
    let relay_sock_str = relay_sock.clone();
    ...
    let vm_thread = std::thread::spawn(move || -> Result<()> {
        ...
        unsafe {
            ...
            // ProxyFs rootfs (VirtioFS via vsock relay)
            let tag_cstr = CString::new("rootfs")?;
            let socket_cstr = CString::new(relay_sock_clone)?;
            if krun_add_virtiofs_proxy(ctx, tag_cstr.as_ptr(), socket_cstr.as_ptr()) < 0 {
                bail!("krun_add_virtiofs_proxy failed");
            }
    ...
    // Wait for relay socket, then start proxy-fsd
    info!("waiting for ProxyFs relay socket: {}...", relay_sock_str);
    let relay_path = Path::new(&relay_sock_str);
    for _ in 0..120 {
        if relay_path.exists() { break; }
        std::thread::sleep(Duration::from_millis(500));
    }
    if !relay_path.exists() {
        bail!("ProxyFs relay socket did not appear: {}", relay_sock_str);
    }

    info!("starting proxy-fsd in podman-machine...");
    let mount_output = Command::new("podman")
        .args(["machine", "ssh", &machine,
            "podman", "image", "mount", &opts.image])
        .output()?;
    let merged = String::from_utf8_lossy(&mount_output.stdout).trim().to_string();
    if merged.is_empty() {
        bail!("podman image mount returned empty path");
    }
    info!("container rootfs mounted: {}", merged);

    let fsd_cmd = format!(
        "nohup /private/tmp/proxy-fsd-linux --root {} --vsock-connect 5001 > /tmp/proxy-fsd.log 2>&1 &",
        merged
    );
    let _fsd = Command::new("podman")
        .args(["machine", "ssh", &machine, &fsd_cmd])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()?;
...
fn find_bcvk_fsd_socket(machine_name: &str) -> Result<String> {
    let tmpdir = std::env::var("TMPDIR")
        .unwrap_or_else(|_| "/tmp".to_string());
    let sock_path = format!("{}/podman/{}-bcvk-fsd", tmpdir.trim_end_matches('/'), machine_name);
    Ok(sock_path)
}
