...
pub fn ensure_internal_switch(name: &str, host_ip: &str, prefix_len: u8) -> Result<SwitchInfo> {
    ...
    // Check/create switch with retry (VMMS parallel creation race: 0x800700B7)
    let mut created = false;
    for attempt in 0..3 {
        let result = run_ps(&format!(
            "$sw = Get-VMSwitch -Name '{}' -EA SilentlyContinue; \
             if ($sw) {{ 'EXISTS' }} else {{ \
             New-VMSwitch -Name '{}' -SwitchType Internal | Out-Null; 'CREATED' }}",
            name, name
        ));
        match result {
            Ok(ref s) if s.contains("EXISTS") => break,
            Ok(ref s) if s.contains("CREATED") => {
                created = true;
                break;
            }
            Err(e) if attempt < 2 => {
                debug!(
                    "switch creation attempt {} failed: {}, retrying...",
                    attempt + 1,
                    e
                );
                std::thread::sleep(std::time::Duration::from_secs(2));
            }
            Err(e) => bail!("switch creation failed after 3 attempts: {}", e),
            _ => break,
        }
    }
...
mod tests {
    ...
    fn test_full_vm_lifecycle() {
        ...
        std::thread::sleep(std::time::Duration::from_secs(2));
        let switch_gone = run_ps(&format!(
            "if (Get-VMSwitch -Name '{}' -EA SilentlyContinue) {{ 'FOUND' }} else {{ 'GONE' }}",
            sw_name
        ))
        .map(|s| s.contains("GONE"))
        .unwrap_or(true);
        eprintln!(
