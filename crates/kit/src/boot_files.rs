...
pub fn extract_boot_files(image: &str) -> Result<BootFiles> {
    ...
    // Write nbd-vsock binary to temp file, mount into container via -v
    let tmp_bin = std::env::temp_dir().join("bcvk-nbd-vsock");
    std::fs::write(&tmp_bin, NBD_VSOCK_BIN)?;
    ...
    let vol_mount = format!("{}:/tmp/nbd-vsock-host:ro", tmp_bin.display());
